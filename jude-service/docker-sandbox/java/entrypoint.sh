...
SOURCE_FILE="/sandbox/Solution.java"
...
# Extract the main class name from the Java file
CLASS_NAME=$(grep -oP '(?<=public class )\w+' "$SOURCE_FILE" | head -1)
if [ -z "$CLASS_NAME" ]; then
    echo "ERROR: Could not find public class name in Java file" > "$ERROR_FILE"
    exit 1
fi

echo "Compiling Java code..." >> "$ERROR_FILE"
...
if timeout -s SIGTERM "${TIME_LIMIT}s" java -Xmx"$MEMORY_LIMIT_JVM" -Xms16m "$CLASS_NAME" < "$INPUT_FILE" > "$OUTPUT_FILE" 2>> "$ERROR_FILE"; then
    EXIT_CODE=$?
